...
pub(crate) enum LockFreeHistogramError {
...
impl LockFreeHistogram {
    ...
    pub(crate) fn increment(
        ...
        if let Err(error) = self.histogram_pool[hot_idx].increment(value) {
    ...
    pub(crate) fn snapshot(&self) -> Result<Histogram, Arc<dyn std::error::Error + Send + Sync>> {
        // Lock the "writers" mutex.
        let _guard = self.snapshot_mutex.lock();
        if _guard.is_err() {
            return Err(Arc::new(LockFreeHistogramError::Mutex));
        }
        ...
        for bucket in snapshot.into_iter() {
            if let Err(error) = self.histogram_pool[hot_idx].add(bucket.start(), bucket.count()) {
                return Err(Arc::new(error));
            }
        }
