...
/// # Fields
///
/// * `alpha` - smoothing factor, a value between 0 and 1
/// * `uncounted` - atomic counter to keep track of the number of requests
/// * `is_initialized` - atomic boolean to check if the EWMA has been initialized
/// * `rate` - atomic value representing the current rate of requests per second
#[derive(Debug)]
